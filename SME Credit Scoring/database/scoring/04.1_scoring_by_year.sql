...
financial_metrics AS  -- (1 слой)Создвю запрос, в котором посчитаю финансовые показатели для комапний
(
SELECT 
...
scoring_query AS( -- (2 слой)Создаю второй запрос, который будет рассчитывать скоринговую сумму в зависимости от отклонений метрик от необходимых значений
SELECT -- Важно! Я ввел систему градации: есть три порога значений, которые отсеивают "неудачные" метрики. Числовые характеристики можно менять по желанию
	...
	financial_metrics.`ROS, %` as "ROS, %", -- Финансовые показатели ниже добавляю в SELECT для того, чтобы в итоговой витрине их тоже отобразить
	financial_metrics.`ROE, %` as "ROE, %",
	financial_metrics.`ROA, %` as "ROA, %",
	financial_metrics.`EBITDA_Margin, %` as "EBITDA_Margin, %",
	financial_metrics.`Долг/выручка, %` as "Долг/выручка, %",
	financial_metrics.`Срок погашения долга, лет` as "Срок погашения долга, лет",
	financial_metrics.`DSCR (покрытие долга)` as "DSCR (покрытие долга)",
	financial_metrics.`Финансовый рычаг` as "Финансовый рычаг",
	financial_metrics.`Текущая ликвидность` as "Текущая ликвидность",
	financial_metrics.`Доля собственных средств в активах` as "Доля собственных средств в активах",
	financial_metrics.`Доля просрочки, %` as "Доля просрочки, %",
	financial_metrics.`Доля закредитованности у других банков, %` as "Доля закредитованности у других банков, %",
	financial_metrics.`Краткосрочные/долгосрочные долги, %` as  "Краткосрочные/долгосрочные долги, %"
FROM 
	financial_metrics
),
final_scoring AS( -- (3 слой)Создаю третий запрос, который будет выносить вердикт по компании в зависимости от скорингого бала, полученного в предыдущем запросе
SELECT
	...
	CASE 
		...
		ELSE "Отказ"
	END as "Решение ",
	scoring_query.`report_year` as "Выбранный год",
	COALESCE(scoring_query.`ROS, %`,0) as "ROS, %",
	COALESCE(scoring_query.`ROE, %`,0) as "ROE, %",
	COALESCE(scoring_query.`ROA, %`,0) as "ROA, %",
	COALESCE(scoring_query.`EBITDA_Margin, %`,0) as "EBITDA_Margin, %",
	COALESCE(scoring_query.`Долг/выручка, %`,0) as "Долг/выручка, %",
	COALESCE(scoring_query.`Срок погашения долга, лет`,0) as "Срок погашения долга, лет",
	COALESCE(scoring_query.`DSCR (покрытие долга)`,0) as "DSCR (покрытие долга)",
	COALESCE(scoring_query.`Финансовый рычаг`,0) as "Финансовый рычаг",
	COALESCE(scoring_query.`Текущая ликвидность`,0) as "Текущая ликвидность",
	COALESCE(scoring_query.`Доля собственных средств в активах`,0) as "Доля собственных средств в активах",
	COALESCE(scoring_query.`Доля просрочки, %`,0) as "Доля просрочки, %",
	COALESCE(scoring_query.`Доля закредитованности у других банков, %`,0) as "Доля закредитованности у других банков, %",
	COALESCE(scoring_query.`Краткосрочные/долгосрочные долги, %`,0) as  "Краткосрочные/долгосрочные долги, %"
FROM
	scoring_query
