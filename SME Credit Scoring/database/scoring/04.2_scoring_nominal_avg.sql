...
financial_metrics AS  -- (1 слой)Создвю запрос, в котором посчитаю финансовые показатели для комапний для каждого года
(
SELECT 
...
SELECT
	...
	CASE 
		...
		ELSE "Отказ"
	END as "Решение ",
	COALESCE(ROUND((scoring_query.`ROS, %`),2),0) as "Среднее номинальное ROS, %",
	COALESCE(ROUND((scoring_query.`ROE, %`),2),0) as "Среднее номинальное ROE, %",
	COALESCE(ROUND((scoring_query.`ROA, %`),2),0) as "Среднее номинальное ROA, %",
	COALESCE(ROUND((scoring_query.`EBITDA_Margin, %`),2),0) as "Среднее номинальное EBITDA_Margin, %",
	COALESCE(ROUND((scoring_query.`Долг/выручка, %`),2),0) as "Среднее номинальное Долг/выручка, %",
	COALESCE(ROUND((scoring_query.`Срок погашения долга, лет`),2),0) as "Среднее номинальное Срок погашения долга, лет",
	COALESCE(ROUND((scoring_query.`DSCR (покрытие долга)`),2),0) as "Среднее номинальное DSCR (покрытие долга)",
	COALESCE(ROUND((scoring_query.`Финансовый рычаг`),2),0) as "Средний номинальный Финансовый рычаг",
	COALESCE(ROUND((scoring_query.`Текущая ликвидность`),2),0) as "Средняя номинальная Текущая ликвидность",
	COALESCE(ROUND((scoring_query.`Доля собственных средств в активах`),2),0) as "Средняя номинальная Доля собственных средств в активах",
	COALESCE(ROUND((scoring_query.`Доля просрочки, %`),2),0) as "Средняя номинальная Доля просрочки, %",
	COALESCE(ROUND((scoring_query.`Доля закредитованности у других банков, %`),2),0) as "Средняя номинальная Доля закредитованности у других банков, %",
	COALESCE(ROUND((scoring_query.`Краткосрочные/долгосрочные долги, %`),2),0) as  "Средние номинальные Краткосрочные/долгосрочные долги, %"
FROM
	scoring_query
